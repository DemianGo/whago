...
export interface TypingSimulationOptions {
  // Se deve enviar presence "composing"
  showTyping?: boolean;
  
  // Se deve simular pausas durante digitação
  simulatePauses?: boolean;
  
  // Probabilidade de fazer uma pausa (0-1)
  pauseProbability?: number;
  
  // Se deve revisar antes de enviar
  reviewBeforeSend?: boolean;
  
  // Se deve ficar online após enviar
  stayOnlineAfter?: boolean;
}
...
export class TypingSimulator {
  ...
  constructor(
    ...
    this.chipId = chipId;
    this.timing = new HumanTiming(tenantId, chipId, timingProfile);
  }

  /**
   * Envia mensagem com simulação de digitação humana
   */
  public async sendMessageHumanLike(
    jid: string,
    text: string,
    ...
    try {
      console.log(
        `[TypingSimulator] ${this.chipId.substring(0, 8)} → ${jid} ` +
      ...
      console.log(
        `[TypingSimulator] ✅ SUCESSO | Duração total: ${result.totalDuration}ms ` +
      ...
      console.error(
        `[TypingSimulator] ❌ ERRO após ${result.totalDuration}ms:`,
        error
      );
   ...
   */
  private splitTypingIntoChunks(
    text: string,
    pauseProbability: number
  ): Array<{ duration: number; pause: boolean }> {
    const chunks: Array<{ duration: number; pause: boolean }> = [];
  ...
  public async sendMultipleMessages(
    ...
    console.log(
      `[TypingSimulator] 📨 Enviando ${messages.length} mensagens para ${jid}`
    );
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      console.log(
        `[TypingSimulator] 📬 Mensagem ${i + 1}/${messages.length}`
      );
      ...
      if (i < messages.length - 1) {
        ...
        console.log(
          `[TypingSimulator] ⏳ Aguardando ${betweenDelay}ms antes da próxima mensagem...`
        );
    ...
    console.log(
      `[TypingSimulator] ✅ Concluído | ${successCount}/${messages.length} enviadas | ` +
   ...
   */
  public async simulateReadingMessages(
    messages: Array<{ key: proto.IMessageKey; text: string }>
  ): Promise<void> {
    console.log(
      `[TypingSimulator] 📖 Simulando leitura de ${messages.length} mensagens`
    );
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      console.log(
        `[TypingSimulator] 👀 Lendo mensagem ${i + 1}/${messages.length} ` +
   ...
   */
  public async simulateViewingStatus(statusCount: number = 1): Promise<void> {
