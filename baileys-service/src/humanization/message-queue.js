...
exports.messageQueueManager = exports.MessageQueueManager = exports.MessageQueue = void 0;
const typing_simulator_1 = require("./typing-simulator");
/**
...
class MessageQueue {
    constructor(socket, tenantId, chipId, timingProfile) {
        ...
        this.MAX_PROCESSING_TIME = 300000; // 5 minutos por mensagem
        this.chipId = chipId;
        this.tenantId = tenantId;
        this.simulator = new typing_simulator_1.TypingSimulator(socket, tenantId, chipId, timingProfile);
        console.log(`[MessageQueue] Fila criada para Tenant ${tenantId} | Chip ${chipId.substring(0, 8)}`);
    }
    /**
     * Adiciona mensagem à fila
     */
    async enqueue(jid, text, options, priority = 'normal') {
        // Verificar limite da fila
        if (this.queue.length >= this.MAX_QUEUE_SIZE) {
            const error = new Error(`Fila cheia (${this.MAX_QUEUE_SIZE} mensagens). Aguarde processamento.`);
            console.error(`[MessageQueue] ${this.chipId.substring(0, 8)} ❌ Fila cheia!`);
            throw error;
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.sortQueue();
            console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} ➕ Mensagem enfileirada ` +
    ...
    async processQueue() {
        if (this.processing) {
            console.warn(`[MessageQueue] ${this.chipId.substring(0, 8)} ⚠️ Já está processando`);
            return;
        }
        this.processing = true;
        console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} 🚀 Iniciando processamento ` +
            `(${this.queue.length} na fila)`);
        while (this.queue.length > 0) {
            const message = this.queue.shift();
            const startTime = Date.now();
            try {
                console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} 📤 Processando ` +
                    `${message.id} → ${message.jid} | "${message.text.substring(0, 30)}..."`);
                // Timeout de segurança
                const timeoutPromise = new Promise((_, reject) => {
                    setTimeout(() => reject(new Error('Timeout ao enviar mensagem')), this.MAX_PROCESSING_TIME);
                });
                // Enviar com simulação humana
                const resultPromise = this.simulator.sendMessageHumanLike(message.jid, message.text, message.options);
                ...
                console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} ✅ ${message.id} enviado ` +
                ...
                console.error(`[MessageQueue] ${this.chipId.substring(0, 8)} ❌ Erro ao processar ${message.id}:`, error);
                // Rejeitar promise
                message.reject(error);
            }
            // Log de progresso
            if (this.queue.length > 0) {
                console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} 📊 Progresso: ` +
                    `${this.stats.totalProcessed} enviadas | ${this.queue.length} restantes`);
            }
        }
        this.processing = false;
        console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} ✅ Fila vazia | ` +
     ...
     */
    clear(reason = 'Fila limpa manualmente') {
        console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} 🧹 Limpando ${this.queue.length} mensagens ` +
            `(motivo: ${reason})`);
        while (this.queue.length > 0) {
            const message = this.queue.shift();
    ...
    pause() {
        if (this.processing) {
            console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} ⏸️  Fila pausada ` +
    ...
    resume() {
        if (!this.processing && this.queue.length > 0) {
            console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} ▶️  Retomando processamento ` +
    ...
    remove(messageId) {
        ...
        if (index !== -1) {
            ...
            console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} ➖ Mensagem ${messageId} removida`);
            return true;
        }
        return false;
    }
    ...
    changeTimingProfile(profileName) {
        ...
        console.log(`[MessageQueue] ${this.chipId.substring(0, 8)} 🔄 Perfil alterado para: ${profileName}`);
    }
    /**
...
class MessageQueueManager {
     ...
     */
    removeQueue(tenantId, chipId) {
        const queueKey = `${tenantId}:${chipId}`;
        const queue = this.queues.get(queueKey);
        if (queue) {
            queue.clear('Chip removido');
            this.queues.delete(queueKey);
            console.log(`[MessageQueueManager] ➖ Fila removida: ${queueKey} | ` +
                `Total de filas: ${this.queues.size}`);
            return true;
        }
        return false;
