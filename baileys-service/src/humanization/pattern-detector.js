...
exports.globalPatternDetector = exports.PatternDetector = void 0;
/**
 * Detector de padrões
 */
class PatternDetector {
    constructor() {
        this.events = [];
        this.MAX_EVENTS = 1000; // Manter últimos 1000 eventos
    }
    /**
     * Registra um evento
     */
     ...
     */
    analyzeTimingPatterns(analysis) {
        ...
        if (coefficientOfVariation < 0.3) {
            analysis.detectedPatterns.push(`Timing de criação muito regular (CV: ${(coefficientOfVariation * 100).toFixed(1)}%)`);
            analysis.warnings.push('Intervalos entre criações muito uniformes - WhatsApp pode detectar');
        }
        ...
        if (fastCreations / intervals.length > 0.3) {
            analysis.detectedPatterns.push(`${fastCreations} criações muito rápidas (< 1min)`);
            analysis.warnings.push('Muitas sessões criadas rapidamente - aumentar delays');
        }
    }
    /**
     * Analisa distribuição por hora do dia
     */
    analyzeHourlyDistribution(analysis) {
        ...
        analysis.metrics.hourlyDistribution = analysis.metrics.hourlyDistribution.map(count => (count / total) * 100);
        ...
        if (maxHourlyPercent > 30) {
            ...
            analysis.detectedPatterns.push(`${maxHourlyPercent.toFixed(1)}% dos eventos concentrados às ${peakHour}h`);
            analysis.warnings.push('Distribuição horária muito concentrada - variar horários de criação');
        }
        // Detectar ausência completa em horários (não-humano)
        const zeroHours = analysis.metrics.hourlyDistribution.filter(p => p === 0).length;
        if (zeroHours > 8) {
            analysis.detectedPatterns.push(`${zeroHours} horas sem nenhuma atividade`);
            analysis.warnings.push('Ausência total em muitas horas - não parece humano');
        }
    }
    /**
     * Analisa intervalos entre ações
     */
    analyzeActionIntervals(analysis) {
        ...
        if (coefficientOfVariation < 0.4) {
            analysis.detectedPatterns.push(`Intervalos entre ações muito regulares (CV: ${(coefficientOfVariation * 100).toFixed(1)}%)`);
            analysis.warnings.push('Ações muito uniformes - adicionar mais variação');
        }
    }
    /**
     * Analisa distribuição por tenant
     */
    analyzeTenantDistribution(analysis) {
        ...
        if (top3Percent > 70 && uniqueTenants > 3) {
            analysis.detectedPatterns.push(`${top3Percent.toFixed(1)}% dos eventos concentrados em 3 tenants`);
            analysis.warnings.push('Atividade muito concentrada em poucos tenants');
        }
    }
    /**
     * Calcula score de diversidade geral (0-100)
     */
    calculateDiversityScore(analysis) {
        let score = 100;
    ...
    generateRecommendations(analysis) {
        ...
        if (analysis.metrics.timingVariance < 0.5) {
            analysis.recommendations.push('🎲 Aumentar variação nos delays de criação (usar jitter > 30%)');
        }
        // Distribuição horária
        const maxHourly = Math.max(...analysis.metrics.hourlyDistribution);
        if (maxHourly > 20) {
            analysis.recommendations.push('⏰ Distribuir criações ao longo do dia (usar ActivitySimulator)');
        }
        const zeroHours = analysis.metrics.hourlyDistribution.filter(p => p === 0).length;
        if (zeroHours > 6) {
            analysis.recommendations.push('🌙 Criar algumas sessões em horários variados (incluir madrugada)');
        }
        // Ações
        if (analysis.metrics.actionIntervalStdDev > 0 && analysis.metrics.actionIntervalStdDev < 120000) {
            analysis.recommendations.push('📝 Aumentar variação nos intervalos entre ações (usar perfis de timing diferentes)');
        }
        // Score geral
        if (analysis.diversityScore < 50) {
            analysis.recommendations.push('⚠️ CRÍTICO: Diversidade muito baixa - revisar toda a estratégia de humanização');
        }
        else if (analysis.diversityScore < 70) {
            analysis.recommendations.push('⚡ Diversidade moderada - aplicar algumas melhorias sugeridas');
        }
        else if (analysis.diversityScore >= 90) {
            analysis.recommendations.push('✅ Excelente diversidade - manter estratégia atual');
        }
    }
    /**
     ...
     */
    clearOldEvents(olderThanMs = 86400000) {
        const cutoff = Date.now() - olderThanMs;
        const initialLength = this.events.length;
        this.events = this.events.filter(e => e.timestamp.getTime() > cutoff);
        const removed = initialLength - this.events.length;
        console.log(`[PatternDetector] 🧹 Removidos ${removed} eventos antigos ` +
...
// Singleton global
exports.globalPatternDetector = new PatternDetector();
