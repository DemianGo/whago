...
exports.HumanTiming = exports.TIMING_PROFILES = void 0;
/**
 * Perfis de timing - muitas opções para alta variação
 */
exports.TIMING_PROFILES = {
 ...
 */
class HumanTiming {
    constructor(tenantId, chipId, profileName) {
        this.tenantId = tenantId;
        this.chipId = chipId;
        // Selecionar perfil (se não especificado, escolhe aleatoriamente)
        if (profileName && exports.TIMING_PROFILES[profileName]) {
            this.profile = exports.TIMING_PROFILES[profileName];
        }
        else {
            this.profile = this.selectRandomProfile();
        }
        console.log(`[HumanTiming] Tenant ${tenantId} | Chip ${chipId.substring(0, 8)} ` +
    ...
    selectRandomProfile() {
        ...
        for (const [key, weight] of Object.entries(weights)) {
            random -= weight;
            if (random <= 0) {
                return exports.TIMING_PROFILES[key];
            }
        }
        return exports.TIMING_PROFILES.normal;
    }
    /**
    ...
    getTypingDelay(textLength) {
        ...
        console.log(`[HumanTiming] ${this.chipId.substring(0, 8)} - Digitando "${textLength} chars": ${finalDelay}ms ` +
     ...
     */
    getOnlinePresenceDelay() {
        const delay = this.randomDelay(this.profile.onlinePresence);
        console.log(`[HumanTiming] ${this.chipId.substring(0, 8)} - Online por: ${delay}ms ` +
    ...
    changeProfile(profileName) {
        if (exports.TIMING_PROFILES[profileName]) {
            this.profile = exports.TIMING_PROFILES[profileName];
            console.log(`[HumanTiming] ${this.chipId.substring(0, 8)} - ` +
                `Perfil alterado para: ${this.profile.name}`);
        }
        else {
            console.warn(`[HumanTiming] Perfil "${profileName}" não existe. ` +
     ...
     */
    async waitForTyping(textLength) {
...
exports.HumanTiming = HumanTiming;
exports.default = HumanTiming;
